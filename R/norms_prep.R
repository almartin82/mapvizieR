...
norms_students_wide_to_long <- function(norms=norms_students_2011) {
  ...
  f2w$growth_season <- 'Fall to Winter'
  f2s$growth_season <- 'Fall to Spring'
  f2f$growth_season <- 'Fall to Fall'
  s2s$growth_season <- 'Spring to Spring'
  w2s$growth_season <- 'Winter to Spring'
  
  #rename
  standardized_names <- c(
    'measurementscale', 'startgrade', 'startrit', 'typical_growth', 
    'reported_growth', 'std_dev_of_expectation', 'growth_season'
  )
